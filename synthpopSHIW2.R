...
newDirectory = sprintf("%s-AreaREAL2: %s-%s-%s---%s", DataSource, Area, Method, sizzz, Sys.time())
...
controlOnMistakes = c(TRUE, TRUE, FALSE, FALSE, TRUE, FALSE)
...
for(r in 1:length(unique(df1$IREG))){
  region=sort(unique(df1$IREG))[r]
  print("reg")
  print(region)
  print(nrow(df1[df1$IREG==region,]))
  print(nrow(df2[df2$IREG==region,]))
  print(length(intersect(df1[df1$IREG==region,idColumn], df2[df2$IREG==region,idColumn]))) #/min(nrow(df1[df1$IREG==region,]),nrow(encodedB[encodedB$IREG==region,])))
}
...
length(intersect(encodedA$ID,encodedB$ID))

nrow(encodedA)
nrow(encodedB)

regions = sort(unique(encodedA$IREG))

for(r in 1:length(regions)){
  region=regions[r]
  print("reg")
  print(region)
  print(nrow(encodedA[encodedA$IREG==region,]))
  print(nrow(encodedB[encodedB$IREG==region,]))
  print(length(intersect(encodedA[encodedA$IREG==region,idColumn], encodedB[encodedB$IREG==region,idColumn]))/min(nrow(encodedA[encodedA$IREG==region,]),nrow(encodedB[encodedB$IREG==region,])))
}
...
for(iterator in 1:5){
  ...
  subsample_size = as.integer(sizzz*nbrRealRecordsB)
  ...
  #
  # syntheticSample=as.integer(sizzz*nbrRealRecordsB)
  ...
  # 
  # syntheticSample=as.integer(sizzz*nbrRealRecordsA)
  # syntheticNewA = syn(encodedA[,PIVs], k=syntheticSample)
  # syntheticNewA = syntheticNewA$syn
  # rownames(syntheticNewA) = 1:nrow(syntheticNewA)
  # columnsToAdd = colnames(encodedA)[(!colnames(encodedA) %in% PIVs) & (!colnames(encodedA) %in% c("localIndex","source"))]
  # for(l in 1:length(columnsToAdd)){
  #   syntheticNewA[,columnsToAdd[l]] = rep(NA, nrow(syntheticNewA))
  # }
  # syntheticNewA$localIndex = 1:nrow(syntheticNewA)
  # syntheticNewA$localIndex = syntheticNewA$localIndex + nrow(encodedA)
  # syntheticNewA$source = "synthetic"
  # 
  # for(p in 1:length(PIVs)){
  #   piv = PIVs[p]
  #   syntheticNewB[,piv] = as.integer(syntheticNewB[,piv])
  #   syntheticNewA[,piv] = as.integer(syntheticNewA[,piv])
  # }
  #
  
  for(p in 1:length(PIVs)){
    piv = PIVs[p]
    encodedB[,piv] = as.integer(encodedB[,piv])
    encodedA[,piv] = as.integer(encodedA[,piv])
  }
  
  # Newdata = synthesise("synthpop", encodedA, encodedB, PIVs, as.integer(sizzz*nbrRealRecordsB))
  NewA = encodedA # syntheticNewA # Newdata$NewA
  NewB =encodedB # syntheticNewB # Newdata$NewB
  ...
  write.csv(DeltaResult, file.path(newDirectory, sprintf("DeltaResult_FastLinkavant_%s_%s_%s_%s.csv", iterator, nbrRealRecordsA, nbrRealRecordsB, Sys.time())))
  if(nrow(DeltaResult)>1){
  ...
  write.csv(DeltaResult, file.path(newDirectory, sprintf("DeltaResult_FlexRLavant_%s_%s_%s_%s.csv", iterator, nbrRealRecordsA, nbrRealRecordsB, Sys.time())))
  if(nrow(DeltaResult)>1){
    DeltaProba = DeltaResult$probaLink
    fdrhat = 1 - sum(DeltaProba[DeltaProba>0.5])/sum(DeltaProba>0.5)
