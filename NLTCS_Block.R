...
df1 = read.csv("NLTCSData/df1982.csv", row.names = 1)
df2 = read.csv("NLTCSData/df1994.csv", row.names = 1)

# Blocking = "None", "REG_CODE"
if(Blocking == "None"){
  print("No blocking.")
  print("BlockingValue is set to NA")
  BlockingValue = "NA"
  
  PIVs_config = list( REG_CODE      = list(stable = TRUE),
                      SEX          = list(stable = TRUE),
                      DOB_YY       = list(stable = TRUE),
                      DOB_MM       = list(stable = TRUE) )
  
}else if(Blocking == "REG_CODE"){
  print("Blocking REG_CODE.")
  print("REG_CODE values:")
  print(unique(c(df1$REG_CODE,df2$REG_CODE)))
  print("Blocking on:")
  print(BlockingValue)
  
  df1 = df1[df1$REG_CODE==as.integer(BlockingValue), ]
  df2 = df2[df2$REG_CODE==as.integer(BlockingValue), ]
  
  PIVs_config = list( ST_CODE      = list(stable = TRUE),
                      SEX          = list(stable = TRUE),
                      DOB_YY       = list(stable = TRUE),
                      DOB_MM       = list(stable = TRUE) )
}else{
  print("Blocking should take value in None, REG_CODE")
}
...
controlOnMistakes = c(TRUE, TRUE, TRUE, TRUE)
